"""Automatic connectivity preparation for structural plates and beams.
...
def _candidate_pairs(geometry: GeometryModel) -> list[tuple[int, int]]:
    """Broad-phase AABB sweep; zero-thickness plates still overlap exactly."""

    records = [
        (face_id, *_face_bounds(geometry, face_id))
        for face_id in geometry.faces
    ]
    records.sort(key=lambda item: (float(item[1][0]), item[0]))
    candidates: list[tuple[int, int]] = []
    active: list[tuple[int, np.ndarray, np.ndarray]] = []
    for face_id, lower, upper in records:
        scale = max(float(np.max(np.abs(np.concatenate((lower, upper))))), 1.0)
        tolerance = 1.0e-9 * scale
        active = [
            item for item in active if float(item[2][0]) + tolerance >= float(lower[0])
        ]
        for other_id, other_lower, other_upper in active:
            pair_scale = max(
                scale,
                float(np.max(np.abs(np.concatenate((other_lower, other_upper))))),
                1.0,
            )
            tol = 1.0e-9 * pair_scale
            if all(
                float(upper[axis]) + tol >= float(other_lower[axis])
                and float(other_upper[axis]) + tol >= float(lower[axis])
                for axis in (1, 2)
            ):
                candidates.append((other_id, face_id))
        active.append((face_id, lower, upper))
    return candidates
    ...
    for face_id in (first, second):
        for item in geometry.faces[face_id].loop:
            if all(
                ...
                <= 1.0e-8 * max(float(np.linalg.norm(point)), 1.0)
...
def _planar_face(
    ...
    if singular[-1] > 1.0e-8 * max(float(singular[0]), 1.0):
        return None
        ...
        for original in original_beams:
            for reference in geometry.resolve_ref(EntityRef("edge", original)):
                ...
                for face_id in tuple(geometry.faces):
                    ...
                    if parameter <= 1.0e-9 or parameter >= 1.0 - 1.0e-9:
                        continue
        ...
        for source_id in current:
            ...
            for vertex_id in (source.start, source.end):
                ...
                for target_id in current:
                    ...
                    if (
                        distance <= 1.0e-8 * scale
                        and 1.0e-9 < parameter < 1.0 - 1.0e-9
                    ):
    ...
    while True:
        ...
        for index, first_id in enumerate(current):
            ...
            for second_id in current[index + 1 :]:
                ...
                if abs(denominator) <= 1.0e-12 * max(a * c, 1.0):
                ...
                if not (
                    -1.0e-9 <= first_parameter <= 1.0 + 1.0e-9
                    and -1.0e-9 <= second_parameter <= 1.0 + 1.0e-9
                ):
                    continue
                first_point = p0 + first_parameter * u
                second_point = q0 + second_parameter * v
                if float(np.linalg.norm(first_point - second_point)) > 1.0e-8 * scale:
                    continue
                if 1.0e-9 < first_parameter < 1.0 - 1.0e-9:
                    geometry.split_edge(first_id, float(first_parameter))
                    split_count += 1
                    changed = True
                    break
                if 1.0e-9 < second_parameter < 1.0 - 1.0e-9:
    ...
    while True:
        ...
        for first, second in _candidate_pairs(working):
            ...
            if _is_one_sided_shell_junction(working, first, second):
                ...
                continue
            try:
                intersect_faces(working, first, second, fragment=True)
