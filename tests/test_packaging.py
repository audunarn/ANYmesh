...
def test_release_metadata_is_0_3_0_alpha() -> None:
    project = _pyproject()["project"]
...
def test_release_workflows_pin_geometry_and_disabled_native_cell() -> None:
    ...
    assert ci.count("--expect-version 0.3.0 --require-native") == 1
    ...
    assert "name: ANYmesher-0.3.0-pypi-distributions" in publish
    ...
    assert 'name: ANYmesher-0.3.0-release-bundle' in publish
    assert 'sdist = root / "anymesher-0.3.0.tar.gz"' in publish
    assert 'root.glob("anymesher-0.3.0-*.whl")' in publish
    assert '"version": "0.3.0"' in publish
    assert "ANYmesher-0.3.0-SHA256SUMS.txt" in publish
    assert "ANYmesher-0.3.0-release-manifest.json" in publish
    ...
    assert publish.count("--expect-version 0.3.0 --require-native") == 1
