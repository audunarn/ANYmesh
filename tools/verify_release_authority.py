...
def _canonical_requirement(value: str) -> str:
    ...
    if match is None:
        _fail("wheel contains a malformed base requirement")
...
def _verify_wheel(path: Path, distribution: str, version: str) -> None:
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            metadata_raw = archive.read(metadata[0])
            _metadata_matches(metadata_raw, distribution, version, "wheel")
            ...
            native = [
                info
                for info in infos
                if info.filename.startswith("anymesher/")
                and info.filename.endswith((".pyd", ".so", ".dylib"))
                and not info.is_dir()
            ]
            if len(native) != 1:
                _fail("wheel must contain exactly one native library")
            if any(
                info.filename.endswith((".jws", ".p7s"))
                for info in infos
            ):
                _fail("wheel contains a forbidden signature member")
            records = [
                info
                for info in infos
            ...
            if len(records) != 1:
                _fail("wheel must contain exactly one RECORD member")
            ...
            if (
                ...
                or {row[0] for row in rows} != set(files)
            ):
                _fail("wheel RECORD member set differs")
            for member, encoded_hash, encoded_size in rows:
                if member == record_name:
                    if encoded_hash or encoded_size:
                        _fail("wheel RECORD self-row is not blank")
                    continue
                ...
                if (
                    ...
                    or encoded_size != str(len(raw))
                ):
                    _fail("wheel RECORD integrity differs")
    except (OSError, zipfile.BadZipFile) as exc:
        _fail(f"wheel is malformed: {exc}")


def _verify_wheel_matrix(
    names: list[str],
    distribution: str,
    version: str,
) -> None:
    normalized = _canonical_name(distribution).replace("-", "_")
    pattern = re.compile(
        re.escape(f"{normalized}-{version}-")
        + r"(cp(?:311|312|313|314))-([A-Za-z0-9_.]+)-"
        + r"([A-Za-z0-9_.]+)\.whl"
    )
    ...
    for name in names:
        ...
        if platform_tag == "win_amd64":
            platform = "Windows"
        elif "manylinux" in platform_tag and "x86_64" in platform_tag:
            platform = "Linux"
        elif platform_tag.startswith("macosx_"):
            platform = "macOS"
        else:
            _fail(f"wheel platform is outside the frozen matrix: {name}")
        platforms[platform].append(python_tag)
    expected = {"cp311", "cp312", "cp313", "cp314"}
    for platform, interpreters in platforms.items():
        if len(interpreters) != 4 or set(interpreters) != expected:
            _fail(
                f"{platform} wheel matrix differs: "
                f"{sorted(interpreters)}"
            )
...
def verify(arguments: argparse.Namespace) -> None:
    ...
    normalized_distribution = _canonical_name(
        arguments.distribution
    ).replace("-", "_")
    if arguments.sdist != (
        f"{normalized_distribution}-{arguments.version}.tar.gz"
    ):
        _fail("expected sdist filename differs")
    ...
    if list(artifacts) != sorted(artifacts):
        _fail("release ledger artifact rows are not canonically ordered")
    wheel_names = [
        name for name in artifacts if name.endswith(".whl")
    ]
    sdist_names = [
        name for name in artifacts if name.endswith(".tar.gz")
    ]
    if (
        len(wheel_names) != 12
        or sdist_names != [arguments.sdist]
        or set(wheel_names) | set(sdist_names) != set(artifacts)
    ):
        _fail("release ledger does not bind the 12-wheel-plus-sdist set")
    _verify_wheel_matrix(
        wheel_names,
        arguments.distribution,
        arguments.version,
    )
